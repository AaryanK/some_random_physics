...
def run(c, truth, outfilename, nmax=-1):
    ...
    if truth != None:
        ...
        # Define function to calculate momentum from kinetic energy using the provided formula
        def momentum_from_kinetic_energy(ke, mass):
         return math.sqrt(ke**2 + mass**2) - mass

        # Define kinetic energy ranges for muons
        min_ke_muon = 0  # MeV
        max_ke_muon = 5000  # MeV
        mid_ke_muon = 2500  # MeV

        # Define kinetic energy ranges for antimuons
        min_ke_antimuon = 0  # MeV
        max_ke_antimuon = 5000  # MeV
        mid_ke_antimuon = 2500  # MeV

        # Define muon mass
        muon_mass = 105.658  # MeV/c^2

        # Calculate momentum ranges for muons
        min_momentum_muon = momentum_from_kinetic_energy(min_ke_muon, muon_mass)
        max_momentum_muon = momentum_from_kinetic_energy(max_ke_muon, muon_mass)
        mid_momentum_muon = momentum_from_kinetic_energy(mid_ke_muon, muon_mass)

        # Calculate momentum ranges for antimuons
        min_momentum_antimuon = momentum_from_kinetic_energy(min_ke_antimuon, muon_mass)
        max_momentum_antimuon = momentum_from_kinetic_energy(max_ke_antimuon, muon_mass)
        mid_momentum_antimuon = momentum_from_kinetic_energy(mid_ke_antimuon, muon_mass)

        # Define histograms for muons
        hist_signed_distance_min_p_muon = ROOT.TH1D("hist_signed_distance_min_p_muon", "Muons signed distance at minimum momentum;True signed distance (mm);Number of muons", 100, -2000, 2000)
        hist_signed_distance_max_p_muon = ROOT.TH1D("hist_signed_distance_max_p_muon", "Muons signed distance at maximum momentum;True signed distance (mm);Number of muons", 100, -2000, 2000)
        hist_signed_distance_mid_p_muon = ROOT.TH1D("hist_signed_distance_mid_p_muon", "Muons signed distance at mid momentum;True signed distance (mm);Number of muons", 100, -2000, 2000)

        # Define histograms for antimuons
        hist_signed_distance_min_p_antimuon = ROOT.TH1D("hist_signed_distance_min_p_antimuon", "Antimuons signed distance at minimum momentum;True signed distance (mm);Number of antimuons", 100, -2000, 2000)
        hist_signed_distance_max_p_antimuon = ROOT.TH1D("hist_signed_distance_max_p_antimuon", "Antimuons signed distance at maximum momentum;True signed distance (mm);Number of antimuons", 100, -2000, 2000)
        hist_signed_distance_mid_p_antimuon = ROOT.TH1D("hist_signed_distance_mid_p_antimuon", "Antimuons signed distance at mid momentum;True signed distance (mm);Number of antimuons", 100, -2000, 2000)

        # Fill histograms for muons
        # Assuming data_min_p_muon, data_max_p_muon, data_mid_p_muon are lists containing signed distance data for respective momentum ranges of muons
        # Replace them with your actual data
        for val in data_min_p_muon:
            hist_signed_distance_min_p_muon.Fill(val)

        for val in data_max_p_muon:
            hist_signed_distance_max_p_muon.Fill(val)

        for val in data_mid_p_muon:
            hist_signed_distance_mid_p_muon.Fill(val)

        # Fill histograms for antimuons
        # Assuming data_min_p_antimuon, data_max_p_antimuon, data_mid_p_antimuon are lists containing signed distance data for respective momentum ranges of antimuons
        # Replace them with your actual data
        for val in data_min_p_antimuon:
            hist_signed_distance_min_p_antimuon.Fill(val)

        for val in data_max_p_antimuon:
            hist_signed_distance_max_p_antimuon.Fill(val)

        for val in data_mid_p_antimuon:
            hist_signed_distance_mid_p_antimuon.Fill(val)

        # Draw histograms for muons
        canvas_muon = ROOT.TCanvas("canvas_muon", "Signed Distance vs Momentum Ranges (Muons)", 800, 600)
        hist_signed_distance_min_p_muon.Draw()
        hist_signed_distance_max_p_muon.Draw("same")
        hist_signed_distance_mid_p_muon.Draw("same")
        legend_muon = ROOT.TLegend(0.7, 0.7, 0.9, 0.9)
        legend_muon.AddEntry(hist_signed_distance_min_p_muon, "Minimum Momentum", "l")
        legend_muon.AddEntry(hist_signed_distance_max_p_muon, "Maximum Momentum", "l")
        legend_muon.AddEntry(hist_signed_distance_mid_p_muon, "Mid Momentum", "l")
        legend_muon.Draw()
        canvas_muon.Draw()

        # Draw histograms for antimuons
        canvas_antimuon = ROOT.TCanvas("canvas_antimuon", "Signed Distance vs Momentum Ranges (Antimuons)", 800, 600)
        hist_signed_distance_min_p_antimuon.Draw()
        hist_signed_distance_max_p_antimuon.Draw("same")
        hist_signed_distance_mid_p_antimuon.Draw("same")
        legend_antimuon = ROOT.TLegend(0.7, 0.7, 0.9, 0.9)
        legend_antimuon.AddEntry(hist_signed_distance_min_p_antimuon, "Minimum Momentum", "l")
        legend_antimuon.AddEntry(hist_signed_distance_max_p_antimuon, "Maximum Momentum", "l")
        legend_antimuon.AddEntry(hist_signed_distance_mid_p_antimuon, "Mid Momentum", "l")
        legend_antimuon.Draw()
        canvas_antimuon.Draw()
    ...
    for i, event in enumerate(c):
        ...
        for branch in truth.GetListOfBranches():
            print(branch)
